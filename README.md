<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Prasad Sudhir Thorat — Cybersecurity &amp; Software Engineering</title>
<meta name="description" content="Prasad Sudhir Thorat — Second-Year Integrated B.Tech CSE student at Sanjivani University. Cybersecurity enthusiast, software developer, AI &amp; cloud computing learner.">
<meta property="og:title" content="Prasad Sudhir Thorat — Portfolio">
<meta property="og:description" content="Cybersecurity Engineer in training | Software Developer | AI &amp; Cloud Computing Enthusiast">
<meta property="og:type" content="website">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='20' fill='%23070B14'/%3E%3Ctext x='50' y='68' font-size='52' text-anchor='middle' fill='%2300E5FF' font-family='monospace'%3EP%3C/text%3E%3C/svg%3E">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
<style>
:root{
  --bg:#070B14; --bg-alt:#0B1220; --surface:rgba(17,24,39,.55); --surface-solid:#101827;
  --border:rgba(0,229,255,.16); --text:#E7EDF6; --muted:#8FA0B8;
  --cyan:#00E5FF; --violet:#7C5CFF; --amber:#FFB020; --green:#29D398;
  --radius:16px; --maxw:1180px;
  --shadow: 0 20px 60px -20px rgba(0,0,0,.6);
}
[data-theme="light"]{
  --bg:#F3F6FB; --bg-alt:#FFFFFF; --surface:rgba(255,255,255,.72); --surface-solid:#FFFFFF;
  --border:rgba(0,110,130,.18); --text:#0B1220; --muted:#55637A;
  --cyan:#0091A8; --violet:#5B3FD9; --amber:#B8790A; --green:#0E9E70;
  --shadow: 0 20px 50px -25px rgba(20,30,50,.25);
}
*{box-sizing:border-box; margin:0; padding:0;}
html{scroll-behavior:smooth;}
body{
  background:var(--bg); color:var(--text); font-family:'Inter',sans-serif;
  line-height:1.6; overflow-x:hidden; transition:background .4s ease, color .4s ease;
}
h1,h2,h3,h4{font-family:'Space Grotesk',sans-serif; font-weight:600; letter-spacing:-.02em;}
.mono{font-family:'JetBrains Mono',monospace;}
a{color:inherit; text-decoration:none;}
img{max-width:100%; display:block;}
.wrap{max-width:var(--maxw); margin:0 auto; padding:0 24px;}
section{padding:110px 0; position:relative;}
.eyebrow{
  font-family:'JetBrains Mono',monospace; font-size:.78rem; letter-spacing:.14em; text-transform:uppercase;
  color:var(--cyan); display:flex; align-items:center; gap:10px; margin-bottom:16px;
}
.eyebrow::before{content:''; width:22px; height:1px; background:var(--cyan); display:inline-block;}
.section-title{font-size:clamp(1.8rem,3.4vw,2.6rem); margin-bottom:14px;}
.section-sub{color:var(--muted); max-width:620px; margin-bottom:56px; font-size:1.02rem;}

/* ---------- Loading / boot sequence ---------- */
#boot{
  position:fixed; inset:0; background:#050810; z-index:9999; display:flex; align-items:center; justify-content:center;
  flex-direction:column; gap:18px; transition:opacity .6s ease, visibility .6s ease;
}
#boot.hide{opacity:0; visibility:hidden; pointer-events:none;}
.boot-box{
  width:min(520px,86vw); background:#0A0F1C; border:1px solid rgba(0,229,255,.25); border-radius:10px;
  padding:22px 24px; font-family:'JetBrains Mono',monospace; font-size:.82rem; color:#7FE9FF;
  box-shadow:0 0 60px rgba(0,229,255,.08);
}
.boot-box .line{opacity:0; animation:bootline .3s forwards;}
.boot-bar{height:4px; width:100%; background:rgba(255,255,255,.08); border-radius:4px; margin-top:14px; overflow:hidden;}
.boot-bar span{display:block; height:100%; width:0%; background:linear-gradient(90deg,var(--cyan),var(--violet)); animation:bootfill 2.2s ease forwards;}
@keyframes bootline{to{opacity:1;}}
@keyframes bootfill{to{width:100%;}}
.boot-cursor{display:inline-block; width:7px; height:14px; background:var(--cyan); animation:blink 1s steps(2) infinite; vertical-align:middle;}
@keyframes blink{50%{opacity:0;}}

/* ---------- Nav ---------- */
#navbar{
  position:fixed; top:0; left:0; right:0; z-index:500; padding:16px 0; transition:.3s;
}
#navbar.scrolled{background:var(--surface); backdrop-filter:blur(16px); border-bottom:1px solid var(--border); box-shadow:var(--shadow);}
.nav-inner{display:flex; align-items:center; justify-content:space-between; max-width:var(--maxw); margin:0 auto; padding:0 24px;}
.logo{font-family:'JetBrains Mono',monospace; font-weight:700; font-size:1.15rem;}
.logo span{color:var(--cyan);}
.nav-links{display:flex; gap:30px; align-items:center; font-size:.92rem;}
.nav-links a{color:var(--muted); position:relative; padding:4px 0;}
.nav-links a:hover{color:var(--text);}
.nav-links a::after{content:''; position:absolute; left:0; bottom:0; width:0; height:1px; background:var(--cyan); transition:.25s;}
.nav-links a:hover::after{width:100%;}
.nav-actions{display:flex; align-items:center; gap:14px;}
.theme-toggle{
  width:40px; height:40px; border-radius:50%; border:1px solid var(--border); background:var(--surface);
  display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:1rem;
}
.hamburger{display:none; flex-direction:column; gap:5px; cursor:pointer; background:none; border:none;}
.hamburger span{width:24px; height:2px; background:var(--text); display:block;}
#mobile-menu{
  display:none; position:fixed; top:64px; left:16px; right:16px; background:var(--surface-solid);
  border:1px solid var(--border); border-radius:14px; padding:18px; flex-direction:column; gap:14px; z-index:499;
  box-shadow:var(--shadow);
}
#mobile-menu.open{display:flex;}

/* ---------- Buttons ---------- */
.btn{
  font-family:'Inter',sans-serif; font-weight:600; font-size:.92rem; padding:13px 26px; border-radius:10px;
  border:1px solid transparent; cursor:pointer; display:inline-flex; align-items:center; gap:9px; transition:.25s;
}
.btn-primary{background:linear-gradient(120deg,var(--cyan),var(--violet)); color:#04101B;}
.btn-primary:hover{filter:brightness(1.12); transform:translateY(-2px);}
[data-theme="light"] .btn-primary{color:#fff;}
.btn-ghost{background:var(--surface); border:1px solid var(--border); color:var(--text);}
.btn-ghost:hover{border-color:var(--cyan); transform:translateY(-2px);}
.btn-sm{padding:9px 16px; font-size:.82rem;}

/* ---------- Hero ---------- */
#hero{padding-top:170px; padding-bottom:120px; min-height:100vh; display:flex; align-items:center; position:relative;}
#particles{position:absolute; inset:0; z-index:0; opacity:.55;}
.hero-grid{display:grid; grid-template-columns:1.15fr .85fr; gap:60px; align-items:center; position:relative; z-index:2; width:100%;}
.hero-name{font-size:clamp(2.3rem,5vw,3.6rem); line-height:1.05; margin-bottom:18px;}
.hero-name .accent{background:linear-gradient(120deg,var(--cyan),var(--violet)); -webkit-background-clip:text; background-clip:text; color:transparent;}
.typed-line{font-family:'JetBrains Mono',monospace; font-size:1.1rem; color:var(--muted); margin-bottom:22px; min-height:1.6em;}
.typed-line::before{content:'> '; color:var(--cyan);}
#typed-cursor{display:inline-block; width:9px; height:1.1em; background:var(--cyan); vertical-align:text-bottom; animation:blink 1s steps(2) infinite;}
.hero-desc{color:var(--muted); max-width:520px; margin-bottom:34px; font-size:1.02rem;}
.hero-btns{display:flex; flex-wrap:wrap; gap:14px; margin-bottom:44px;}
.hero-stats{display:flex; gap:34px; flex-wrap:wrap;}
.hero-stats .stat b{display:block; font-family:'Space Grotesk',sans-serif; font-size:1.5rem; color:var(--cyan);}
.hero-stats .stat span{font-size:.8rem; color:var(--muted); font-family:'JetBrains Mono',monospace;}

.hero-photo-wrap{position:relative; display:flex; justify-content:center;}
.photo-frame{
  position:relative; width:min(340px,80vw); aspect-ratio:1/1.08; border-radius:28px; overflow:hidden;
  border:1px solid var(--border); box-shadow:var(--shadow); background:var(--surface-solid);
}
.photo-frame img{width:100%; height:100%; object-fit:cover;}
.photo-frame::after{
  content:''; position:absolute; inset:0; border-radius:28px;
  background:linear-gradient(160deg, rgba(0,229,255,.18), transparent 45%);
  pointer-events:none;
}
.photo-glow{position:absolute; width:70%; height:70%; background:radial-gradient(circle,var(--cyan),transparent 70%); opacity:.18; filter:blur(40px); top:10%; left:15%; z-index:-1;}
.badge-chip{
  position:absolute; background:var(--surface); border:1px solid var(--border); backdrop-filter:blur(10px);
  padding:9px 14px; border-radius:30px; font-family:'JetBrains Mono',monospace; font-size:.76rem;
  display:flex; align-items:center; gap:7px; box-shadow:var(--shadow);
}
.badge-chip.b1{top:6%; left:-8%;}
.badge-chip.b2{bottom:16%; right:-10%;}
.badge-chip.b3{bottom:-4%; left:8%;}
.dot{width:7px; height:7px; border-radius:50%; background:var(--green); box-shadow:0 0 8px var(--green);}

/* ---------- Glass card base ---------- */
.card{
  background:var(--surface); border:1px solid var(--border); border-radius:var(--radius);
  backdrop-filter:blur(14px); box-shadow:var(--shadow); padding:28px;
}

/* ---------- About ---------- */
#about .about-grid{display:grid; grid-template-columns:1.2fr .8fr; gap:50px; align-items:start;}
#about p{color:var(--muted); margin-bottom:16px;}
.terminal-card{font-family:'JetBrains Mono',monospace; font-size:.86rem;}
.terminal-card .t-head{display:flex; gap:6px; margin-bottom:16px;}
.terminal-card .t-head span{width:10px; height:10px; border-radius:50%; display:inline-block;}
.t-head .r{background:#FF5F56;} .t-head .y{background:#FFBD2E;} .t-head .g{background:#27C93F;}
.terminal-card .row{display:flex; justify-content:space-between; padding:9px 0; border-bottom:1px dashed var(--border); color:var(--muted);}
.terminal-card .row b{color:var(--text); font-weight:500;}
.terminal-card .row:last-child{border:none;}

/* ---------- Skills ---------- */
.skill-tabs{display:flex; flex-wrap:wrap; gap:10px; margin-bottom:40px;}
.skill-tab{
  padding:10px 20px; border-radius:30px; border:1px solid var(--border); background:var(--surface);
  cursor:pointer; font-size:.85rem; font-family:'JetBrains Mono',monospace; color:var(--muted); transition:.2s;
}
.skill-tab.active{background:linear-gradient(120deg,var(--cyan),var(--violet)); color:#04101B; border-color:transparent;}
.skill-panel{display:none;}
.skill-panel.active{display:grid; grid-template-columns:repeat(2,1fr); gap:22px 40px;}
.skill-item .s-top{display:flex; justify-content:space-between; font-size:.9rem; margin-bottom:8px;}
.skill-item .s-top span.level{color:var(--muted); font-family:'JetBrains Mono',monospace; font-size:.75rem;}
.skill-bar{height:7px; border-radius:6px; background:rgba(255,255,255,.08); overflow:hidden;}
[data-theme="light"] .skill-bar{background:rgba(0,0,0,.08);}
.skill-bar span{display:block; height:100%; width:0; border-radius:6px; background:linear-gradient(90deg,var(--cyan),var(--violet)); transition:width 1.2s ease;}

/* ---------- Timeline ---------- */
.timeline{position:relative; padding-left:34px;}
.timeline::before{content:''; position:absolute; left:6px; top:6px; bottom:6px; width:2px; background:linear-gradient(var(--cyan),var(--violet));}
.t-item{position:relative; padding-bottom:34px;}
.t-item:last-child{padding-bottom:0;}
.t-item::before{content:''; position:absolute; left:-34px; top:3px; width:13px; height:13px; border-radius:50%; background:var(--bg); border:2px solid var(--cyan);}
.t-item .tag{display:inline-block; font-family:'JetBrains Mono',monospace; font-size:.72rem; color:var(--amber); background:rgba(255,176,32,.12); padding:3px 10px; border-radius:20px; margin-bottom:8px;}

/* ---------- Internship ---------- */
.intern-card{display:grid; grid-template-columns:auto 1fr; gap:26px; align-items:flex-start;}
.intern-icon{width:56px; height:56px; border-radius:14px; background:linear-gradient(135deg,var(--cyan),var(--violet)); display:flex; align-items:center; justify-content:center; font-size:1.4rem; flex-shrink:0;}
.intern-points{display:grid; grid-template-columns:repeat(2,1fr); gap:10px; margin-top:16px;}
.intern-points div{color:var(--muted); font-size:.9rem; display:flex; gap:8px; align-items:flex-start;}
.intern-points div::before{content:'▹'; color:var(--cyan);}

/* ---------- Certifications ---------- */
.cert-controls{display:flex; flex-wrap:wrap; gap:12px; margin-bottom:34px; align-items:center; justify-content:space-between;}
.cert-filters{display:flex; flex-wrap:wrap; gap:8px;}
.cert-filters button{
  padding:8px 16px; border-radius:20px; border:1px solid var(--border); background:var(--surface);
  font-size:.78rem; font-family:'JetBrains Mono',monospace; cursor:pointer; color:var(--muted);
}
.cert-filters button.active{background:var(--cyan); color:#04101B; border-color:transparent;}
#cert-search{
  padding:10px 16px; border-radius:20px; border:1px solid var(--border); background:var(--surface); color:var(--text);
  font-family:'Inter',sans-serif; font-size:.85rem; min-width:220px;
}
.cert-grid{display:grid; grid-template-columns:repeat(3,1fr); gap:18px;}
.cert-card{padding:20px; font-size:.88rem;}
.cert-card .cert-cat{font-family:'JetBrains Mono',monospace; font-size:.7rem; color:var(--cyan); text-transform:uppercase; margin-bottom:8px; letter-spacing:.08em;}
.cert-card h4{font-size:.98rem; margin-bottom:6px;}
.cert-card p{color:var(--muted); font-size:.8rem;}

/* ---------- Projects ---------- */
.proj-filters{display:flex; gap:10px; margin-bottom:36px; flex-wrap:wrap;}
.proj-filters button{padding:9px 18px; border-radius:20px; border:1px solid var(--border); background:var(--surface); color:var(--muted); font-family:'JetBrains Mono',monospace; font-size:.8rem; cursor:pointer;}
.proj-filters button.active{background:linear-gradient(120deg,var(--cyan),var(--violet)); color:#04101B; border-color:transparent;}
.proj-grid{display:grid; grid-template-columns:repeat(3,1fr); gap:24px;}
.proj-card{overflow:hidden; padding:0; transition:.3s;}
.proj-card:hover{transform:translateY(-6px); box-shadow:0 30px 70px -25px rgba(0,229,255,.18);}
.proj-thumb{height:150px; display:flex; align-items:center; justify-content:center; font-family:'JetBrains Mono',monospace; font-size:2.2rem; color:#04101B; font-weight:700; position:relative; overflow:hidden;}
.proj-thumb::after{content:attr(data-tag); position:absolute; bottom:8px; left:12px; font-size:.65rem; letter-spacing:.1em; opacity:.75;}
.proj-body{padding:22px;}
.proj-body h4{margin-bottom:8px; font-size:1.05rem;}
.proj-body p{color:var(--muted); font-size:.85rem; margin-bottom:14px;}
.proj-tech{display:flex; flex-wrap:wrap; gap:6px; margin-bottom:16px;}
.proj-tech span{font-size:.7rem; font-family:'JetBrains Mono',monospace; padding:3px 9px; border-radius:12px; background:rgba(0,229,255,.1); color:var(--cyan);}
.proj-links{display:flex; gap:10px;}

/* ---------- Achievements ---------- */
.ach-grid{display:grid; grid-template-columns:repeat(4,1fr); gap:22px; text-align:center;}
.ach-item .num{font-family:'Space Grotesk',sans-serif; font-size:2.6rem; background:linear-gradient(120deg,var(--cyan),var(--violet)); -webkit-background-clip:text; background-clip:text; color:transparent;}
.ach-item .lbl{color:var(--muted); font-size:.85rem; margin-top:6px; font-family:'JetBrains Mono',monospace;}

/* ---------- Gallery ---------- */
.gallery-grid{display:grid; grid-template-columns:repeat(4,1fr); gap:16px;}
.gallery-item{
  aspect-ratio:1; border-radius:14px; border:1px solid var(--border); display:flex; align-items:center; justify-content:center;
  font-family:'JetBrains Mono',monospace; font-size:.78rem; text-align:center; padding:14px; cursor:pointer; position:relative; overflow:hidden;
}
.gallery-item span{position:relative; z-index:2;}
#lightbox{
  position:fixed; inset:0; background:rgba(3,6,12,.9); z-index:1000; display:none; align-items:center; justify-content:center; padding:30px;
}
#lightbox.open{display:flex;}
#lightbox .lb-box{max-width:520px; text-align:center;}
#lightbox .lb-close{position:absolute; top:24px; right:34px; font-size:1.6rem; cursor:pointer; color:#fff;}

/* ---------- Contact ---------- */
.contact-card{display:grid; grid-template-columns:repeat(3,1fr); gap:18px;}
.contact-item{text-align:center; padding:26px 18px;}
.contact-item .ic{font-size:1.5rem; margin-bottom:12px;}
.contact-item h4{font-size:.95rem; margin-bottom:4px;}
.contact-item p{color:var(--muted); font-size:.8rem; margin-bottom:14px; word-break:break-word;}

/* ---------- Floating dock ---------- */
#dock{position:fixed; right:22px; bottom:22px; display:flex; flex-direction:column; gap:10px; z-index:400;}
#dock a{
  width:46px; height:46px; border-radius:50%; background:var(--surface-solid); border:1px solid var(--border);
  display:flex; align-items:center; justify-content:center; font-size:1.1rem; box-shadow:var(--shadow); transition:.2s;
}
#dock a:hover{background:var(--cyan); color:#04101B; transform:scale(1.08);}

/* ---------- Back to top / progress ---------- */
#progress-bar{position:fixed; top:0; left:0; height:3px; background:linear-gradient(90deg,var(--cyan),var(--violet)); z-index:600; width:0%;}
#totop{
  position:fixed; left:22px; bottom:22px; width:44px; height:44px; border-radius:50%; background:var(--surface-solid);
  border:1px solid var(--border); display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:400;
  opacity:0; pointer-events:none; transition:.3s;
}
#totop.show{opacity:1; pointer-events:auto;}

/* ---------- Footer ---------- */
footer{padding:44px 0; border-top:1px solid var(--border); text-align:center; color:var(--muted); font-size:.85rem;}
footer .f-links{display:flex; justify-content:center; gap:22px; margin-bottom:18px;}

/* ---------- Reveal on scroll ---------- */
.reveal{opacity:0; transform:translateY(26px); transition:opacity .7s ease, transform .7s ease;}
.reveal.in{opacity:1; transform:none;}

@media (prefers-reduced-motion:reduce){
  *{animation:none !important; transition:none !important;}
}

@media (max-width:960px){
  .hero-grid{grid-template-columns:1fr;}
  .hero-photo-wrap{order:-1;}
  #about .about-grid{grid-template-columns:1fr;}
  .skill-panel.active{grid-template-columns:1fr;}
  .cert-grid{grid-template-columns:repeat(2,1fr);}
  .proj-grid{grid-template-columns:repeat(2,1fr);}
  .ach-grid{grid-template-columns:repeat(2,1fr);}
  .gallery-grid{grid-template-columns:repeat(2,1fr);}
  .contact-card{grid-template-columns:1fr;}
  .nav-links{display:none;}
  .hamburger{display:flex;}
}
@media (max-width:600px){
  section{padding:76px 0;}
  #hero{padding-top:140px;}
  .badge-chip{display:none;}
  .proj-grid, .cert-grid, .gallery-grid{grid-template-columns:1fr;}
  .intern-points{grid-template-columns:1fr;}
}
</style>
</head>
<body data-theme="dark">

<div id="progress-bar"></div>

<!-- Boot / loading screen -->
<div id="boot">
  <div class="boot-box">
    <div class="line" style="animation-delay:.1s">$ whoami</div>
    <div class="line" style="animation-delay:.5s">prasad_thorat — b.tech(cse) · sanjivani university</div>
    <div class="line" style="animation-delay:1s">$ initializing_portfolio --secure</div>
    <div class="line" style="animation-delay:1.4s">loading skills, projects, certifications...<span class="boot-cursor"></span></div>
    <div class="boot-bar"><span></span></div>
  </div>
</div>

<!-- Nav -->
<nav id="navbar">
  <div class="nav-inner">
    <a href="#hero" class="logo">PT<span>_</span></a>
    <div class="nav-links">
      <a href="#about">About</a>
      <a href="#skills">Skills</a>
      <a href="#education">Education</a>
      <a href="#certifications">Certifications</a>
      <a href="#projects">Projects</a>
      <a href="#achievements">Achievements</a>
      <a href="#contact">Contact</a>
    </div>
    <div class="nav-actions">
      <button class="theme-toggle" id="theme-toggle" aria-label="Toggle theme">🌙</button>
      <a class="btn btn-primary btn-sm" id="resume-btn-nav" href="Prasad_Thorat_Resume.pdf" download>Resume</a>
      <button class="hamburger" id="hamburger" aria-label="Menu"><span></span><span></span><span></span></button>
    </div>
  </div>
  <div id="mobile-menu">
    <a href="#about">About</a>
    <a href="#skills">Skills</a>
    <a href="#education">Education</a>
    <a href="#certifications">Certifications</a>
    <a href="#projects">Projects</a>
    <a href="#achievements">Achievements</a>
    <a href="#contact">Contact</a>
  </div>
</nav>

<!-- Hero -->
<section id="hero">
  <canvas id="particles"></canvas>
  <div class="wrap hero-grid">
    <div>
      <div class="eyebrow">Second-Year B.Tech CSE · Sanjivani University</div>
      <h1 class="hero-name">Prasad Sudhir <span class="accent">Thorat</span></h1>
      <div class="typed-line"><span id="typed-text"></span><span id="typed-cursor"></span></div>
      <p class="hero-desc">I'm passionate about cybersecurity, AI, cloud computing and software engineering — building secure, scalable solutions and learning by doing, one project at a time.</p>
      <div class="hero-btns">
        <a href="Prasad_Thorat_Resume.pdf" download class="btn btn-primary">⬇ Download Resume</a>
        <a href="#projects" class="btn btn-ghost">View Projects</a>
        <a href="#contact" class="btn btn-ghost">Contact Me</a>
      </div>
      <div class="hero-stats">
        <div class="stat"><b>18</b><span>CERTIFICATIONS</span></div>
        <div class="stat"><b>06</b><span>EVENTS &amp; COMPETITIONS</span></div>
        <div class="stat"><b>01</b><span>INTERNSHIP</span></div>
      </div>
    </div>
    <div class="hero-photo-wrap">
      <div class="photo-glow"></div>
      <div class="photo-frame">
        <img src="data:image/jpeg;base64," alt="Prasad Sudhir Thorat">
      </div>
      <div class="badge-chip b1"><span class="dot"></span> Cybersecurity</div>
      <div class="badge-chip b2"><span class="dot"></span> AI &amp; GenAI</div>
      <div class="badge-chip b3"><span class="dot"></span> Cloud Computing</div>
    </div>
  </div>
</section>

<!-- About -->
<section id="about">
  <div class="wrap">
    <div class="eyebrow reveal">About</div>
    <h2 class="section-title reveal">Building secure, thoughtful software</h2>
    <div class="about-grid">
      <p class="reveal" style="font-size:1.05rem;">
        I am Prasad Sudhir Thorat, a Second-Year Integrated B.Tech (Computer Science &amp; Engineering) student at Sanjivani University.
        I'm passionate about cybersecurity, artificial intelligence, cloud computing, web development, and software engineering — I enjoy
        building applications, exploring emerging technologies, and solving real-world problems through code.
      </p>
      <p class="reveal">
        I keep improving through internships, certifications, workshops, and hands-on projects. My goal is to grow into a skilled
        cybersecurity engineer and software developer who builds secure, scalable, and impactful digital solutions.
      </p>
      <div class="card terminal-card reveal">
        <div class="t-head"><span class="r"></span><span class="y"></span><span class="g"></span></div>
        <div class="row"><span>location</span><b>Kopargaon, Maharashtra, IN</b></div>
        <div class="row"><span>university</span><b>Sanjivani University</b></div>
        <div class="row"><span>focus</span><b>Cybersecurity · AI · Cloud</b></div>
        <div class="row"><span>status</span><b style="color:var(--green)">● Open to opportunities</b></div>
      </div>
    </div>
  </div>
</section>

<!-- Skills -->
<section id="skills" style="background:var(--bg-alt);">
  <div class="wrap">
    <div class="eyebrow reveal">Skills</div>
    <h2 class="section-title reveal">What I work with</h2>
    <p class="section-sub reveal">A snapshot of where I currently stand — self-rated, growing every semester.</p>
    <div class="skill-tabs">
      <button class="skill-tab active" data-tab="lang">Languages</button>
      <button class="skill-tab" data-tab="cyber">Cybersecurity</button>
      <button class="skill-tab" data-tab="cloud">Cloud</button>
      <button class="skill-tab" data-tab="ai">AI</button>
      <button class="skill-tab" data-tab="tools">Tools</button>
    </div>

    <div class="skill-panel active" data-panel="lang">
      <div class="skill-item"><div class="s-top"><span>C</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="65"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Python</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="70"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>HTML</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="82"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>CSS</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="78"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>JavaScript</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="68"></span></div></div>
    </div>

    <div class="skill-panel" data-panel="cyber">
      <div class="skill-item"><div class="s-top"><span>Network Security</span><span class="level">Familiar</span></div><div class="skill-bar"><span data-w="55"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Digital Forensics</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="62"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>IP Addressing</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="75"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Subnetting</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="75"></span></div></div>
    </div>

    <div class="skill-panel" data-panel="cloud">
      <div class="skill-item"><div class="s-top"><span>AWS ECS</span><span class="level">Familiar</span></div><div class="skill-bar"><span data-w="55"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Cloud Computing</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="60"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>KodeKloud Labs</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="62"></span></div></div>
    </div>

    <div class="skill-panel" data-panel="ai">
      <div class="skill-item"><div class="s-top"><span>Claude AI</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="80"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>ChatGPT</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="80"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Google AI Workspace</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="65"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Generative AI</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="68"></span></div></div>
    </div>

    <div class="skill-panel" data-panel="tools">
      <div class="skill-item"><div class="s-top"><span>Git</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="60"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>GitHub</span><span class="level">Practicing</span></div><div class="skill-bar"><span data-w="65"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>VS Code</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="85"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Microsoft Excel</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="78"></span></div></div>
      <div class="skill-item"><div class="s-top"><span>Canva</span><span class="level">Confident</span></div><div class="skill-bar"><span data-w="80"></span></div></div>
    </div>
  </div>
</section>

<!-- Education -->
<section id="education">
  <div class="wrap">
    <div class="eyebrow reveal">Education</div>
    <h2 class="section-title reveal">Academic journey</h2>
    <div class="timeline" style="max-width:700px;">
      <div class="t-item reveal">
        <div class="tag">IN PROGRESS · 2ND YEAR</div>
        <h3>Integrated B.Tech — Computer Science &amp; Engineering</h3>
        <p style="color:var(--muted); margin-top:6px;">Sanjivani University, Kopargaon, Maharashtra</p>
        <p style="color:var(--muted); margin-top:6px; font-size:.9rem;">Currently a second-year student, building a foundation across programming, systems, and cybersecurity fundamentals.</p>
      </div>
    </div>
  </div>
</section>

<!-- Internship -->
<section id="internship" style="background:var(--bg-alt);">
  <div class="wrap">
    <div class="eyebrow reveal">Experience</div>
    <h2 class="section-title reveal">Internship</h2>
    <div class="card intern-card reveal">
      <div class="intern-icon">🛡️</div>
      <div>
        <h3>Cybersecurity Intern</h3>
        <p style="color:var(--muted); margin-top:4px;">InternsPort Innovation Pvt. Ltd. &nbsp;·&nbsp; Feb 2026 – Apr 2026</p>
        <div class="intern-points">
          <div>Completed a 2-month cybersecurity internship</div>
          <div>Earned a Letter of Recommendation from the Head of Operations</div>
          <div>Strengthened problem-solving &amp; analytical thinking</div>
          <div>Practiced team collaboration &amp; clear communication</div>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- Certifications -->
<section id="certifications">
  <div class="wrap">
    <div class="eyebrow reveal">Credentials</div>
    <h2 class="section-title reveal">Certifications</h2>
    <p class="section-sub reveal">18 certifications across cloud, programming, AI, cybersecurity, and communication.</p>
    <div class="cert-controls">
      <div class="cert-filters">
        <button class="active" data-filter="all">All</button>
        <button data-filter="Cloud">Cloud</button>
        <button data-filter="Programming">Programming</button>
        <button data-filter="AI">AI &amp; GenAI</button>
        <button data-filter="Cyber">Cybersecurity</button>
        <button data-filter="Soft">Soft Skills</button>
      </div>
      <input type="text" id="cert-search" placeholder="Search certifications...">
    </div>
    <div class="cert-grid" id="cert-grid"><!-- injected by JS --></div>
  </div>
</section>

<!-- Projects -->
<section id="projects" style="background:var(--bg-alt);">
  <div class="wrap">
    <div class="eyebrow reveal">Work</div>
    <h2 class="section-title reveal">Projects</h2>
    <p class="section-sub reveal">A mix of security-minded builds, web apps, and personal experiments.</p>
    <div class="proj-filters">
      <button class="active" data-pf="all">All</button>
      <button data-pf="Security">Security &amp; AI</button>
      <button data-pf="Web">Web Apps</button>
      <button data-pf="Personal">Personal</button>
    </div>
    <div class="proj-grid" id="proj-grid"><!-- injected by JS --></div>
  </div>
</section>

<!-- Achievements -->
<section id="achievements">
  <div class="wrap">
    <div class="eyebrow reveal">Track record</div>
    <h2 class="section-title reveal">Achievements</h2>
    <div class="ach-grid">
      <div class="ach-item reveal"><div class="num" data-count="1">0</div><div class="lbl">INTERNSHIP</div></div>
      <div class="ach-item reveal"><div class="num" data-count="18">0</div><div class="lbl">CERTIFICATIONS</div></div>
      <div class="ach-item reveal"><div class="num" data-count="6">0</div><div class="lbl">EVENTS &amp; COMPETITIONS</div></div>
      <div class="ach-item reveal"><div class="num" data-count="2">0</div><div class="lbl">YEAR OF STUDY</div></div>
    </div>
  </div>
</section>

<!-- Gallery -->
<section id="gallery" style="background:var(--bg-alt);">
  <div class="wrap">
    <div class="eyebrow reveal">Highlights</div>
    <h2 class="section-title reveal">Gallery</h2>
    <p class="section-sub reveal">A few milestones — tap to view.</p>
    <div class="gallery-grid" id="gallery-grid"><!-- injected by JS --></div>
  </div>
</section>

<!-- Contact -->
<section id="contact">
  <div class="wrap">
    <div class="eyebrow reveal">Get in touch</div>
    <h2 class="section-title reveal">Let's build something</h2>
    <p class="section-sub reveal">Open to internships, collaborations, and interesting conversations about security &amp; software.</p>
    <div class="contact-card">
      <div class="card contact-item reveal">
        <div class="ic">📧</div>
        <h4>Email</h4>
        <p>prasad.thorat_25uid@sanjivani.edu.in</p>
        <a class="btn btn-primary btn-sm" href="mailto:prasad.thorat_25uid@sanjivani.edu.in">Email Me</a>
      </div>
      <div class="card contact-item reveal">
        <div class="ic">📞</div>
        <h4>Phone</h4>
        <p>+91 8010989708</p>
        <a class="btn btn-primary btn-sm" href="tel:+918010989708">Call Me</a>
      </div>
      <div class="card contact-item reveal">
        <div class="ic">📍</div>
        <h4>Location</h4>
        <p>Kopargaon, Maharashtra, India</p>
        <a class="btn btn-primary btn-sm" href="https://www.google.com/maps/place/Kopargaon,+Maharashtra" target="_blank" rel="noopener">View Location</a>
      </div>
      <div class="card contact-item reveal">
        <div class="ic">💼</div>
        <h4>LinkedIn</h4>
        <p>prasad-thorat-a38578372</p>
        <a class="btn btn-primary btn-sm" href="https://www.linkedin.com/in/prasad-thorat-a38578372" target="_blank" rel="noopener">LinkedIn Profile</a>
      </div>
      <div class="card contact-item reveal">
        <div class="ic">💻</div>
        <h4>GitHub</h4>
        <p>prasadthorat25uid-arch</p>
        <a class="btn btn-primary btn-sm" href="https://github.com/prasadthorat25uid-arch" target="_blank" rel="noopener">GitHub Profile</a>
      </div>
      <div class="card contact-item reveal">
        <div class="ic">📄</div>
        <h4>Resume</h4>
        <p>Full CV in PDF format</p>
        <a class="btn btn-primary btn-sm" href="Prasad_Thorat_Resume.pdf" download>Download</a>
      </div>
    </div>
  </div>
</section>

<footer>
  <div class="wrap">
    <div class="f-links">
      <a href="mailto:prasad.thorat_25uid@sanjivani.edu.in">Email</a>
      <a href="https://www.linkedin.com/in/prasad-thorat-a38578372" target="_blank" rel="noopener">LinkedIn</a>
      <a href="https://github.com/prasadthorat25uid-arch" target="_blank" rel="noopener">GitHub</a>
    </div>
    <p>© <span id="year"></span> Prasad Sudhir Thorat. Built with curiosity and a lot of coffee.</p>
  </div>
</footer>

<!-- Lightbox -->
<div id="lightbox">
  <span class="lb-close" id="lb-close">&times;</span>
  <div class="lb-box card" id="lb-content"></div>
</div>

<!-- Floating dock -->
<div id="dock">
  <a href="mailto:prasad.thorat_25uid@sanjivani.edu.in" title="Email">📧</a>
  <a href="tel:+918010989708" title="Call">📞</a>
  <a href="https://www.linkedin.com/in/prasad-thorat-a38578372" target="_blank" rel="noopener" title="LinkedIn">💼</a>
  <a href="https://github.com/prasadthorat25uid-arch" target="_blank" rel="noopener" title="GitHub">💻</a>
  <a href="https://www.google.com/maps/place/Kopargaon,+Maharashtra" target="_blank" rel="noopener" title="Location">📍</a>
</div>
<div id="totop">↑</div>

<script>
document.getElementById('year').textContent = new Date().getFullYear();

/* ---------- Boot sequence ---------- */
window.addEventListener('load', ()=>{
  setTimeout(()=>{ document.getElementById('boot').classList.add('hide'); }, 2300);
});

/* ---------- Theme toggle ---------- */
const themeBtn = document.getElementById('theme-toggle');
themeBtn.addEventListener('click', ()=>{
  const body = document.body;
  const isLight = body.getAttribute('data-theme') === 'light';
  body.setAttribute('data-theme', isLight ? 'dark' : 'light');
  themeBtn.textContent = isLight ? '🌙' : '☀️';
});

/* ---------- Nav scroll + mobile menu ---------- */
const navbar = document.getElementById('navbar');
const hamburger = document.getElementById('hamburger');
const mobileMenu = document.getElementById('mobile-menu');
hamburger.addEventListener('click', ()=> mobileMenu.classList.toggle('open'));
mobileMenu.querySelectorAll('a').forEach(a=>a.addEventListener('click', ()=>mobileMenu.classList.remove('open')));

const progressBar = document.getElementById('progress-bar');
const totop = document.getElementById('totop');
window.addEventListener('scroll', ()=>{
  const st = window.scrollY;
  navbar.classList.toggle('scrolled', st > 40);
  totop.classList.toggle('show', st > 500);
  const h = document.documentElement.scrollHeight - window.innerHeight;
  progressBar.style.width = (st / h * 100) + '%';
});
totop.addEventListener('click', ()=> window.scrollTo({top:0, behavior:'smooth'}));

/* ---------- Typed effect ---------- */
const roles = ["Second-Year Integrated B.Tech Student","Cybersecurity Enthusiast","Software Developer","AI & Cloud Computing Learner"];
let rIdx=0, cIdx=0, deleting=false;
const typedEl = document.getElementById('typed-text');
function typeLoop(){
  const word = roles[rIdx];
  if(!deleting){
    cIdx++;
    typedEl.textContent = word.slice(0,cIdx);
    if(cIdx === word.length){ deleting = true; setTimeout(typeLoop, 1400); return; }
  } else {
    cIdx--;
    typedEl.textContent = word.slice(0,cIdx);
    if(cIdx === 0){ deleting = false; rIdx = (rIdx+1)%roles.length; }
  }
  setTimeout(typeLoop, deleting ? 35 : 65);
}
typeLoop();

/* ---------- Particles background (lightweight canvas) ---------- */
const canvas = document.getElementById('particles');
const ctx = canvas.getContext('2d');
let particles = [];
function resizeCanvas(){ canvas.width = canvas.offsetWidth; canvas.height = canvas.offsetHeight; }
function initParticles(){
  resizeCanvas();
  const count = Math.min(70, Math.floor(canvas.width/18));
  particles = Array.from({length:count}, ()=>({
    x:Math.random()*canvas.width, y:Math.random()*canvas.height,
    vx:(Math.random()-.5)*.3, vy:(Math.random()-.5)*.3, r:Math.random()*1.6+.6
  }));
}
function drawParticles(){
  ctx.clearRect(0,0,canvas.width,canvas.height);
  ctx.fillStyle = 'rgba(0,229,255,.7)';
  particles.forEach(p=>{
    p.x+=p.vx; p.y+=p.vy;
    if(p.x<0||p.x>canvas.width) p.vx*=-1;
    if(p.y<0||p.y>canvas.height) p.vy*=-1;
    ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill();
  });
  for(let i=0;i<particles.length;i++){
    for(let j=i+1;j<particles.length;j++){
      const a=particles[i], b=particles[j];
      const d = Math.hypot(a.x-b.x,a.y-b.y);
      if(d<110){
        ctx.strokeStyle = `rgba(0,229,255,${(1-d/110)*.15})`;
        ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.stroke();
      }
    }
  }
  requestAnimationFrame(drawParticles);
}
window.addEventListener('resize', initParticles);
initParticles(); drawParticles();

/* ---------- Skill tabs ---------- */
document.querySelectorAll('.skill-tab').forEach(tab=>{
  tab.addEventListener('click', ()=>{
    document.querySelectorAll('.skill-tab').forEach(t=>t.classList.remove('active'));
    document.querySelectorAll('.skill-panel').forEach(p=>p.classList.remove('active'));
    tab.classList.add('active');
    const panel = document.querySelector(`.skill-panel[data-panel="${tab.dataset.tab}"]`);
    panel.classList.add('active');
    animateBars(panel);
  });
});
function animateBars(panel){
  panel.querySelectorAll('.skill-bar span').forEach(s=>{
    s.style.width='0%';
    requestAnimationFrame(()=> requestAnimationFrame(()=> s.style.width = s.dataset.w + '%'));
  });
}
animateBars(document.querySelector('.skill-panel.active'));

/* ---------- Certifications data + render ---------- */
const certs = [
  {cat:'Cloud', name:'AWS Elastic Container Service (ECS)', issuer:'KodeKloud'},
  {cat:'Cloud', name:'KodeKloud Challenges Completion Certificate', issuer:'KodeKloud'},
  {cat:'Programming', name:'Introduction to Python', issuer:'Infosys Springboard'},
  {cat:'Programming', name:'Python Fundamentals', issuer:'Infosys Springboard'},
  {cat:'Programming', name:'Introduction to C', issuer:'Sololearn'},
  {cat:'Programming', name:'Programming For Beginners: Master the C Language', issuer:'Learn Programming Academy'},
  {cat:'AI', name:'AI Fluency: AI Capabilities & Limitations', issuer:'Anthropic'},
  {cat:'AI', name:'Introduction to Claude Cowork', issuer:'Anthropic'},
  {cat:'AI', name:'Claude 101', issuer:'Anthropic'},
  {cat:'AI', name:'Bring AI to Work Workshop', issuer:'Google Workspace'},
  {cat:'AI', name:'Introduction to Generative AI Studio', issuer:'Simplilearn'},
  {cat:'AI', name:'Generative AI Mastery Workshop', issuer:'NxtWave (OpenAI Academy)'},
  {cat:'AI', name:'AI Tools & ChatGPT Workshop', issuer:'10X/BeLux'},
  {cat:'AI', name:'Microsoft Excel Using AI', issuer:'OfficeMaster'},
  {cat:'Cyber', name:'Cybersecurity Mastery', issuer:'Unstop'},
  {cat:'Cyber', name:'Hands-on Digital Forensics & Investigation Workshop', issuer:'Indian Cyber Club'},
  {cat:'Cyber', name:'SEBI Investor Awareness Test', issuer:'NISM'},
  {cat:'Soft', name:'Communication Skills', issuer:'MindLuster'},
];
const catLabel = {Cloud:'Cloud & Infrastructure', Programming:'Programming', AI:'AI & GenAI', Cyber:'Cybersecurity', Soft:'Soft Skills'};
const certGrid = document.getElementById('cert-grid');
function renderCerts(filter='all', query=''){
  certGrid.innerHTML = '';
  certs.filter(c => (filter==='all'||c.cat===filter) && (c.name.toLowerCase().includes(query)||c.issuer.toLowerCase().includes(query)))
    .forEach(c=>{
      const div = document.createElement('div');
      div.className = 'card cert-card reveal in';
      div.innerHTML = `<div class="cert-cat">${catLabel[c.cat]}</div><h4>${c.name}</h4><p>${c.issuer}</p>`;
      certGrid.appendChild(div);
    });
}
renderCerts();
document.querySelectorAll('.cert-filters button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    document.querySelectorAll('.cert-filters button').forEach(b=>b.classList.remove('active'));
    btn.classList.add('active');
    renderCerts(btn.dataset.filter, document.getElementById('cert-search').value.toLowerCase());
  });
});
document.getElementById('cert-search').addEventListener('input', (e)=>{
  const active = document.querySelector('.cert-filters button.active').dataset.filter;
  renderCerts(active, e.target.value.toLowerCase());
});

/* ---------- Projects data + render ---------- */
const projects = [
  {cat:'Security', tag:'IDS', title:'AI-Based Intrusion Detection System', desc:'Exploring ML-driven approaches to detect network intrusions, backed by a literature review of verified research papers.', tech:['Python','AI/ML','Cybersecurity'], gradient:'135deg,#00E5FF,#7C5CFF'},
  {cat:'Web', tag:'HEALTH', title:'Blood Donor Finder Website', desc:'A web app connecting blood donors with people in need, with search and matching by location and blood group.', tech:['HTML','CSS','JavaScript'], gradient:'135deg,#FF5F6D,#FFC371'},
  {cat:'Web', tag:'HEALTH', title:'Hospital Appointment Website', desc:'A booking interface for hospital appointments with a clean scheduling flow and patient-friendly UI.', tech:['HTML','CSS','JavaScript'], gradient:'135deg,#29D398,#00B4D8'},
  {cat:'Personal', tag:'CREATIVE', title:'Birthday Surprise Website', desc:'A personal, animated webpage built as a birthday surprise — a fun exploration of creative front-end animation.', tech:['HTML','CSS','JS Animation'], gradient:'135deg,#FF7CA3,#7C5CFF'},
  {cat:'Web', tag:'CAMPUS', title:'College Event Website', desc:'An event microsite for a college tech event, covering schedules, registration, and speaker information.', tech:['HTML','CSS','Bootstrap'], gradient:'135deg,#FFB020,#FF5F6D'},
  {cat:'Personal', tag:'PORTFOLIO', title:'Personal Portfolio Website', desc:'This very site — a cybersecurity-themed portfolio built as a single-file, production-ready web experience.', tech:['HTML5','CSS3','JavaScript'], gradient:'135deg,#00E5FF,#5B3FD9'},
];
const projGrid = document.getElementById('proj-grid');
function renderProjects(filter='all'){
  projGrid.innerHTML='';
  projects.filter(p=>filter==='all'||p.cat===filter).forEach(p=>{
    const div = document.createElement('div');
    div.className = 'card proj-card reveal in';
    div.innerHTML = `
      <div class="proj-thumb" data-tag="${p.tag}" style="background:linear-gradient(${p.gradient})">${p.title.split(' ').map(w=>w[0]).slice(0,2).join('')}</div>
      <div class="proj-body">
        <h4>${p.title}</h4>
        <p>${p.desc}</p>
        <div class="proj-tech">${p.tech.map(t=>`<span>${t}</span>`).join('')}</div>
        <div class="proj-links">
          <a class="btn btn-ghost btn-sm" href="https://github.com/prasadthorat25uid-arch" target="_blank" rel="noopener">GitHub ↗</a>
        </div>
      </div>`;
    projGrid.appendChild(div);
  });
}
renderProjects();
document.querySelectorAll('.proj-filters button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    document.querySelectorAll('.proj-filters button').forEach(b=>b.classList.remove('active'));
    btn.classList.add('active');
    renderProjects(btn.dataset.pf);
  });
});

/* ---------- Gallery ---------- */
const galleryItems = [
  {icon:'🏆', title:'Techfest, IIT Bombay', sub:'AI Workshop · Dec 2025'},
  {icon:'🚀', title:'Smart India Hackathon', sub:'Team Code Warriors · Sept 2025'},
  {icon:'📜', title:'Constitution Day Quiz', sub:'Score: 90% · Nov 2025'},
  {icon:'🤖', title:'GenAI Buildathon', sub:'NxtWave / OpenAI Academy · Nov 2025'},
];
const galleryGrid = document.getElementById('gallery-grid');
galleryItems.forEach((g,i)=>{
  const div = document.createElement('div');
  div.className = 'card gallery-item reveal in';
  div.style.background = `linear-gradient(150deg, rgba(0,229,255,.12), rgba(124,92,255,.12))`;
  div.innerHTML = `<span>${g.icon}<br><b style="display:block;margin-top:6px;">${g.title}</b><small style="color:var(--muted)">${g.sub}</small></span>`;
  div.addEventListener('click', ()=>openLightbox(g));
  galleryGrid.appendChild(div);
});
function openLightbox(g){
  document.getElementById('lb-content').innerHTML = `<div style="font-size:2.4rem;">${g.icon}</div><h3 style="margin-top:12px;">${g.title}</h3><p style="color:var(--muted);margin-top:6px;">${g.sub}</p>`;
  document.getElementById('lightbox').classList.add('open');
}
document.getElementById('lb-close').addEventListener('click', ()=>document.getElementById('lightbox').classList.remove('open'));
document.getElementById('lightbox').addEventListener('click', (e)=>{ if(e.target.id==='lightbox') e.currentTarget.classList.remove('open'); });

/* ---------- Reveal on scroll + counters ---------- */
const revealEls = document.querySelectorAll('.reveal');
const counters = document.querySelectorAll('.ach-item .num');
const io = new IntersectionObserver((entries)=>{
  entries.forEach(e=>{
    if(e.isIntersecting){
      e.target.classList.add('in');
      if(e.target.classList.contains('ach-item')){
        const numEl = e.target.querySelector('.num');
        animateCount(numEl);
      }
    }
  });
},{threshold:.2});
revealEls.forEach(el=>io.observe(el));

function animateCount(el){
  if(el.dataset.done) return;
  el.dataset.done = '1';
  const target = parseInt(el.dataset.count,10);
  let cur = 0;
  const step = Math.max(1, Math.ceil(target/40));
  const t = setInterval(()=>{
    cur += step;
    if(cur>=target){ cur=target; clearInterval(t); }
    el.textContent = cur;
  }, 40);
}
</script>
</body>
</html>
